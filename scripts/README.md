...
├── download_moex_events.py  # Загрузка событий с MOEX ISS (deprecated, используй API)
...
make download-all                  # скачать финансы + рыночные данные
make events                        # загрузить IR-календарь и дивиденды в API
make fill-events                   # сгенерировать events.md из API
...
### 0.2. download_moex_events.py (deprecated)

> **Deprecated:** Данные теперь загружаются через Investment API:
> - Дивиденды: `POST /jobs/fetch-events/{TICKER}`
> - IR-календарь: `POST /jobs/fetch-ir-calendar`
> - Makefile: `make events`

Загружает события и дивиденды с MOEX ISS API в локальные файлы (legacy).
...
Генерирует `events.md` из данных Investment API.
...
**Входные данные:**
- `GET /companies/{TICKER}/catalysts?type=event` — IR-календарь из API
- `GET /companies/{TICKER}/dividends` — дивиденды из API
- `russia/macro.md` — заседания ЦБ (локально)
...
**Предварительно:** загрузите данные в API: `make events`
...
**Входные данные:**
- `GET /companies/{TICKER}/dividends` — дивиденды из API (или `data/moex_events.json` как fallback)
