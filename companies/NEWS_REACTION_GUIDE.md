...
**Проверка свежести:** Поле `date` содержит дату и время новости (datetime). Если новость старше 2 торговых дней → **skip** (рынок уже отработал). Исключение: новость вышла в выходные/после закрытия — тогда реакция будет при следующем открытии.
...
Если в статье feeder есть `summary` — используй его для понимания контекста. Если нет — прочитай новость по `url`.
